...
inline bool fieldAt(const std::string& text, size_t position, const char* label, size_t length) {
  // A field is separate from the preceding word: "errormessage:" is not a field.
  // A typed backslash+n may separate fields, but is never decoded in the body.
  const bool boundary = position == 0 || text[position-1] == ' ' || text[position-1] == '\t' ||
    text[position-1] == '\r' || text[position-1] == '\n' ||
    (position >= 2 && text[position-2] == '\\' && text[position-1] == 'n');
  return boundary && text.compare(position, length, label) == 0;
}

inline bool hasField(const std::string& text) {
  for (size_t p=0; p<text.size(); ++p) {
    if (fieldAt(text, p, "title:", 6) || fieldAt(text, p, "message:", 8)) return true;
  }
  return false;
}
...
inline bool parse(const char* channel, const char* raw, Alert& alert) {
  ...
  if (text.compare(0, 6, "title:")) {
    ...
    text = trim(text.substr(separator + 2));
  }
  if (text.compare(0, 6, "title:")) return false;
  size_t messageField = std::string::npos;
  for (size_t p=6; p<text.size(); ++p) {
    if (fieldAt(text, p, "message:", 8)) { messageField = p; break; }
  }
  if (messageField == std::string::npos) return false;
  std::string title = trim(text.substr(6, messageField-6));
  // Accept a literal "\n" only at the title/message boundary, leaving body text intact.
  if (title.size() >= 2 && title.compare(title.size()-2, 2, "\\n") == 0) {
    title = trim(title.substr(0, title.size()-2));
  }
  std::string body = trim(text.substr(messageField+8));
  if (title.empty() || title.size() >= sizeof(alert.title) || body.empty() || body.size() >= sizeof(alert.message)) return false;
  // Keep titles on one line and reject extra fields, including ambiguous inline duplicates.
  // Ordinary multiline message text is still allowed.
  if (title.find_first_of("\r\n") != std::string::npos || hasField(title) || hasField(body)) return false;
  alert = Alert{};
